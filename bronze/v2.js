...
while (true) {
    ...
    for (let potion of potions) {
        ...
        let etapeSuivante;
        if(tourPrecedent && tourPrecedent.potionIdObjectif === potion.id){
            debug('tourPrecedent', tourPrecedent);
            etapeSuivante = tourPrecedent;
            debug('tourPrecedent.recette.suite.key', tourPrecedent.recette.suite ? tourPrecedent.recette.suite.key : 'suite undefined');
            debug('tourPrecedent.recette.precedent.key', tourPrecedent.recette.precedent ? tourPrecedent.recette.precedent.key : 'tourPrecedent.recette.precedent undefined');
            debug('tourPrecedent.recette.suivant.key', tourPrecedent.recette.suivant? tourPrecedent.recette.suivant.key : 'tourPrecedent.recette.suivant undefined');
            etapeSuivante.cout ? etapeSuivante.recette.cout-- : 1;

            etapeSuivante.suite = etapeSuivante.suite.precedent;
        } else {
            etapeSuivante = aEtoile(monInventaire.inv, potion, sorts, monHeuristique);
        }
        etapeSuivante.gain = potion.prix;
        etapeSuivante.rentabilite = etapeSuivante.gain / etapeSuivante.cout;
        etapeSuivante.potionId = potion.id;
    ...
    debug('recetteToDo', recetteToDo);
    // in the first league: BREW <id> | WAIT; later: BREW <id> | CAST <id> [<times>] | LEARN <id> | REST | WAIT

    tourPrecedent.action = `${recetteToDo.suite.sort.type} ${recetteToDo.suite.sort.id}`;
    tourPrecedent.potionIdObjectif = recetteToDo.potionId;
    tourPrecedent.recette = recetteToDo;
    console.log(tourPrecedent.action);
}
